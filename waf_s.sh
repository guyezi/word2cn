...
bt_domain=https://api.loli.host/cr
...
{	
	...
	wget -O $pluginPath/btwaf_httpd_main.py $bt_domain/btwaf_httpd_main.py -T 5
	...
	if [ ! -f $btwaf_httpd_path/rule/url.json ];then
		...
		\cp -a -r /tmp/btwaf/rule/referer.json $btwaf_path/rule/referer.json
	fi
	...
	python $pluginPath/btwaf_httpd_main.py zhuzu
	chmod -R  777 /www/server/btwaf/*.json
